...
const ActivityForm = ({ onAddActivity, onCancel }) => {
  ...
  const handlePhotoUpload = (e) => {
    if (isUploading) return; // Don't process if already uploading
  ...
  const handleSubmit = async () => {
    if (isUploading) return; // Don't submit if already uploading
          ...
          <div className="relative inline-block">
            ...
            <button
              ...
              className={`absolute top-1 right-1 bg-white rounded-full p-1 shadow-sm ${
                isUploading ? 'opacity-50 cursor-not-allowed' : 'opacity-70 hover:opacity-100 transition-opacity'
              }`}
              disabled={isUploading}
            >
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className={`px-4 py-2 ${isUploading || (!content.trim() && !activityPhoto) 
              ...
              : 'bg-gradient-to-r from-[#4A2BAF] to-[#5D4EFF] hover:opacity-90'} text-white rounded-lg transition-opacity duration-200 flex items-center`}
          >
            {isUploading ? (
              <>
