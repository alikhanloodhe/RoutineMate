  ...
  try {
    ...
    const query = `
      WITH completed_days AS (
        SELECT ht.habit_id, ht.date
        FROM habit_tracking ht
        JOIN habits h ON ht.habit_id = h.id
        WHERE ht.completed = TRUE AND ht.user_id = $1 AND h.user_id = $1
      ),
      streak_groups AS (
        SELECT  habit_id,  date, date - (ROW_NUMBER() OVER (PARTITION BY habit_id ORDER BY date))::integer AS streak_group
        FROM completed_days
      ),
      streaks AS (
        SELECT habit_id, MIN(date) AS streak_start, MAX(date) AS streak_end, COUNT(*) AS streak_length
        FROM streak_groups
        GROUP BY habit_id, streak_group
      ),
      longest_streaks AS (
        SELECT habit_id, MAX(streak_length) AS longest_streak
        FROM streaks
        GROUP BY habit_id
      ),
      current_streaks AS (
        SELECT habit_id, streak_length AS current_streak
        FROM streaks
        WHERE streak_end = CURRENT_DATE OR streak_end = (CURRENT_DATE - INTERVAL '1 day')::date
      ),
      all_habit_streaks AS (
        SELECT  h.id AS habit_id, COALESCE(cs.current_streak, 0) AS current_streak, COALESCE(ls.longest_streak, 0) AS longest_streak
        FROM habits h
